...
const aesEncrypt = (text, mode, key, iv, format = "base64") => {
    let encrypted = CryptoJS.AES.encrypt(CryptoJS.enc.Utf8.parse(text), CryptoJS.enc.Utf8.parse(key), {
...
const aesDecrypt = (ciphertext, key, iv, format = "base64") => {
    ...
    if (format === "base64") {
        ...
        bytes = CryptoJS.AES.decrypt({ciphertext: CryptoJS.enc.Hex.parse(ciphertext)}, CryptoJS.enc.Utf8.parse(key), {
            iv: CryptoJS.enc.Utf8.parse(iv),
            mode: CryptoJS.mode.ECB,
            padding: CryptoJS.pad.Pkcs7,
        })
