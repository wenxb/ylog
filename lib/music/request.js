...
import {cookieObjToString, cookieToJson, generateRandomChineseIP, toBoolean} from "./index"
import {URLSearchParams} from "node:url"
import {APP_CONF} from "./config"
import {getSettingsByKeys} from "@/utils/server"
// request.debug = true // 开启可看到更详细信息

const genIp = generateRandomChineseIP()
...
const createRequest = (uri, data, options) => {
    return new Promise(async (resolve, reject) => {
        ...
        let ip = options.realIP || options.ip || genIp
        // console.log(ip)
        if (ip) {
        ...
        const setting = await getSettingsByKeys(["music_cookie"])

        const user_cookie = setting.music_cookie || ""
        let cookie = options.cookie || `MUSIC_U=${user_cookie};` || {}
